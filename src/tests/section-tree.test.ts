...
it('should include only its own content', () => {

  const test = ` <1000>\n One <100>\n Twooo <50>\n Three </>\n </>\n </>\n `;

  const plan2 = new WritingPlan(test, new WritingPlanOptions());
  // expect(plan.totalWords).toBe(3)
  // expect(plan2.sections[1].content).toBe('Two')
  expect(plan2.getFirstSection().content).toBe('One');
});
